...
/**
 * A horizontally scrollable {@link ViewGroup} with items populated from an {@link Adapter}. The ViewFlow uses a buffer
 * to store loaded {@link View}s in. The default size of the buffer is 3 elements on both sides of the currently visible
 * {@link View}, making up a total buffer size of 3 * 2 + 1 = 7. The buffer size can be changed using the
 * {@code sidebuffer} xml attribute.
 *
 */
public class ViewFlow extends AdapterView<Adapter>
{
    ...
    public void setSelection(int position)
    {
        ...
        for (int offset = 1; mSideBuffer - offset >= 0; offset++)
        {
            ...
            if (leftIndex >= 0)
                mLoadedViews.addFirst(makeAndAddView(leftIndex,
                        false,
                        (recycleViews.isEmpty() ? null : recycleViews.remove(0))));
            if (rightIndex < mAdapter.getCount())
                mLoadedViews.addLast(makeAndAddView(rightIndex,
                        true,
                        (recycleViews.isEmpty() ? null : recycleViews.remove(0))));
        }
    ...
    private View setupChild(View child, boolean addToEnd, boolean recycle)
    {
        ...
        {
            p =
                    new AbsListView.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        if (recycle)
